export class SegmentTree<T> {
  ...
  constructor(private readonly data: T[], private readonly merge: (a: T, b: T) => T, private readonly defaultValue: T) {
    this.n = data.length;
  ...
  #update(node: number, l: number, r: number, index: number, value: T) {
    if (l === r) {
      this.tree[node] = value;
    } else {
      const mid = Math.floor((l + r) / 2);
      if (index <= mid) {
        this.#update(2 * node + 1, l, mid, index, value);
      } else {
        this.#update(2 * node + 2, mid + 1, r, index, value);
      }
      this.tree[node] = this.merge(this.tree[2 * node + 1], this.tree[2 * node + 2]);
    }
  }
  ...
  #query(node: number, l: number, r: number, left: number, right: number): T {
    if (r < left || l > right) return this.defaultValue;
    if (left <= l && r <= right) return this.tree[node];

    const mid = Math.floor((l + r) / 2);
    const leftVal = this.#query(2 * node + 1, l, mid, left, right);
    const rightVal = this.#query(2 * node + 2, mid + 1, r, left, right);
